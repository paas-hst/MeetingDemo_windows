                                   ...
                                   CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if( HIWORD(xml.m_lpstr) != NULL ) 
	{
        if( *(xml.m_lpstr) == _T('<') ) 
		{
            if( !m_xml.Load(xml.m_lpstr) ) // 从字符串加载
				return NULL;
        }
        else 
		{
            if( !m_xml.LoadFromFile(xml.m_lpstr) ) // 从文件加载
				return NULL;
        }
    }
    else 
...
CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if( pManager ) 
	{
		...
		// Root下可以包含"Image"/"Font"/"Default"/"MultiLanguage"几种子项
		...
		// root必须是"Window"，但window不是control
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) 
	{
		...
		else if( _tcsicmp(pstrClass, _T("TreeNode")) == 0 ) //树控件XML解析
		{
        ...
        // Add children
        if( node.HasChildren() ) // 递归解析，典型场景有Layout下放置控件
        ...
        // Attach to parent
		if( pParent != NULL ) // 因为某些属性和父窗口相关，比如selected，必须先Add到父窗口 
		{
            ...
            else 
			{
                ...
                else
                {
                    ...
                    ASSERT(pContainer);
                    if( pContainer == NULL ) // 父控件如果不是Container？？？ 
						return NULL;

                    if( !pContainer->Add(pControl) ) // 父控件是Container，则添加到Container中
					{
                        pControl->Delete();
                        continue;
                    }
