...
bool CNotifyPump::LoopDispatch(TNotifyUI& msg)
{
	...
	// GetMessageMap为虚函数，从下往上按图索骥寻找消息处理函数
#ifndef UILIB_STATIC
...
bool CWindowWnd::RegisterSuperclass()
{
    ...
    if( !::GetClassInfoEx(NULL, GetSuperClassName(), &wc) ) // 系统定义
	{
        if( !::GetClassInfoEx(CPaintManagerUI::GetInstance(), GetSuperClassName(), &wc) ) // 自定义 
		{
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) // 先于WM_CREATE
	{
    ...
    else 
	{
        ...
        if( uMsg == WM_NCDESTROY && pThis != NULL ) // 后于WM_DESTROY
		{
			// 恢复老的窗口过程
