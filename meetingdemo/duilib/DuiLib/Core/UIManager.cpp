...
CPaintManagerUI* CPaintManagerUI::GetPaintManager(LPCTSTR pstrName)
{
	...
	{
		CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
		if( pManager != NULL && sName == pManager->GetName() ) 
			return pManager;
	}
	return NULL;
}
    ...
    case WM_KEYDOWN:
        {
           ...
           if( wParam == VK_TAB ) 
		   {
               if( m_pFocus && m_pFocus->IsVisible() 
				   && m_pFocus->IsEnabled() 
...
bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
    ...
    case WM_APP + 1: // 异步消息处理
        {
			...
			while( pMsg = static_cast<TNotifyUI*>(m_aAsyncNotify.GetAt(0)) ) 
			{
				...
				// 通过委托订阅
				if (pMsg->pSender != NULL && pMsg->pSender->OnNotify)
    ...
    case WM_CLOSE: // 一堆清理工作
        {
			...
			if( ::GetActiveWindow() == m_hWndPaint ) 
			{
                ...
                if ((GetWindowStyle(m_hWndPaint) & WS_CHILD) !=0 ) 
					hwndParent = GetParent(m_hWndPaint);
    ...
    case WM_PAINT:
        {
            if( m_pRoot == NULL ) // 理论上这种情况不会发生，在加载xml时就会判断？？？
			{
                PAINTSTRUCT ps = { 0 };
                ::BeginPaint(m_hWndPaint, &ps);
				CRenderEngine::DrawColor(m_hDcPaint, ps.rcPaint, 0xFF000000); // 窗口背景涂成黑色
...
{
	ASSERT(pControl);
	if( pControl == NULL || pControl->GetManager() != this || pstrName == NULL || *pstrName == _T('\0')) 
		return false;

	if (pControl->GetName() == pstrName) 
		return true;

	if (NULL != FindControl(pstrName)) // TODO:bug??
		return false;

	m_mNameHash.Remove(pControl->GetName());
	bool bResult = m_mNameHash.Insert(pstrName, pControl);
	if (bResult) 
		pControl->SetName(pstrName);
	return bResult;
}
...
void CPaintManagerUI::SendNotify(TNotifyUI& Msg, bool bAsync /*= false*/, bool bEnableRepeat /*= true*/)
{
    ...
    if( !bAsync ) // 同步Send
	{
        ...
        if( Msg.pSender != NULL ) // 通过委托监听
		{
            if( Msg.pSender->OnNotify ) 
				Msg.pSender->OnNotify(&Msg);
        }

        for( int i = 0; i < m_aNotifiers.GetSize(); i++ ) // 通过INotify监听(消息映射)
		{
