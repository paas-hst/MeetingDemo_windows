...
namespace DuiLib
{
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		DWORD dwCurTextColor = m_dwTextColor;

		if (GetText() == m_sSrcTipValue || GetText() == _T(""))
			dwCurTextColor = m_dwTipValueColor;
		...
		if( IsEnabled() ) {
			CRenderEngine::DrawText(hDC, m_pManager, rc, sText, dwCurTextColor, \
	...
	void CEditUI::SetTipValueColor(LPCTSTR pStrColor)
	{
		...
		LPTSTR pStr = NULL;
		m_dwTipValueColor = _tcstoul(pStrColor, &pStr, 16);
	}
