...
//	}
//	UILIB_COMDAT const DUI_MSGMAP WindowImplBase::messageMap =
//	{ 
//		&WindowImplBase::_GetBaseMessageMap, &WindowImplBase::_messageEntries[0] 
//	};
...
{
	// 收到消息时：
	// pParam->rgrc[0]: 移动后或改变大小后窗口的新坐标
	// pParam->rgrc[1]: 移动前或改变大小前窗口的旧坐标
	// pParam->rgrc[2]: 移动钱或改变大小前客户区的旧坐标
	// 处理消息后：
	// pParam->rgrc[0]: 移动后或改变大小后客户区的新坐标
	// pParam->rgrc[1]: 移动后或改变大小后窗口的新坐标
	// pParam->rgrc[2]: 移动前或改变大小前窗口的旧坐标

	LPRECT pRect=NULL;

	if ( wParam == TRUE) // 其他情况走这里
	{
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	// WS_CLIPSIBLINGS: 不对兄弟窗口进行绘制(被本窗口遮挡的兄弟窗口可以正常显示，谦让策略)
	::SetWindowLong(*this, GWL_STYLE, styleValue | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);

	// 重新设置窗口的位置/尺寸/Z序
	RECT rcClient;
	::GetClientRect(*this, &rcClient);
	// SWP_FRAMECHANGED: 即使窗口尺寸没有改变，也要给窗口发送WM_NCCALCSIZE消息
	// 第二个参数NULL，等同于HWND_TOP，即将窗口置于Z序的顶部
	::SetWindowPos(
		*this, 
		NULL, 
		rcClient.left, 
...
void WindowImplBase::OnClick(TNotifyUI& msg)
{
	...
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MINIMIZE, 0);  // 调用系统处理
		return; 
	}
	else if( sCtrlName == _T("maxbtn"))
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MAXIMIZE, 0); // 调用系统处理
		return; 
	}
	else if( sCtrlName == _T("restorebtn"))
	{ 
		SendMessage(WM_SYSCOMMAND, SC_RESTORE, 0);  // 调用系统处理
		return; 
	}
	return;
}
